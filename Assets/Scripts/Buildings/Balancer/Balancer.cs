...
public class Balancer : DoubleBasicBuilding
{
    ...
    private void Update()
    {
       for(int i = 0; i < Belts.Length; i++) //두 컨베이어 벨트만큼 반복
       {
    ...
    public int CalculateMoveConveyor(int startIndex) //현재 Output이 연결되어 있는 컨베이어 벨트의 Index를 저장
    {
       ...
       {
           if(Belts[(startIndex + i + curCount) % Belts.Length].canMove) //만약 curCount + Index의 벨트가 이동 가능하다면
           {
               return (startIndex + i + curCount) % Belts.Length; //이동 가능한 Index를 반환
           }
       }
